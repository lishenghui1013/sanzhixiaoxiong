...
//跳到拼团编辑页
function spellToEdit(id,src){
    location.href = WST.U('home/goods/spellEdit','id='+id+'&src='+src);
}
/**保存数据**/
function save1(){
	$('#editform').isValid(function(v){
		if(v){
		    ...
		    $.post(WST.U('home/raise/toAdd'),params,function(data,textStatus){
		    	layer.close(loading);
				//console.log(data);
		    	var json = WST.toJson(data);
		    	if(json.status=='1'){
...
function save(){
	$('#editform').isValid(function(v){
		if(v){
			...
			// 商品描述
            var gall = [];
            $('.bjy-filename').each(function(){
                gall.push($(this).val());
            });
            params.goodsDesc = gall.join(',');
            console.log(params);
...
function spellSave(){
    $('#editform').isValid(function(v){
        if(v){
            ...
            params.specmap = specmap.join(',');
            var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(WST.U('home/goods/'+((params.goodsId==0)?"spellToAdd":"spellToEdit")),params,function(data,textStatus){
                console.log(data);
                layer.close(loading);
                var json = WST.toJson(data);
                if(json.status=='1'){
...
//获取拼团商品
function spellSaleByPage(p){
    $('#list').html('<tr><td colspan="11"><img src="'+WST.conf.ROOT+'/wstmart/home/view/default/img/loading.gif">正在加载数据...</td></tr>');
    var params = {};
    params = WST.getParams('.s-query');
    params.key = $.trim($('#key').val());
    params.page = p;
    $.post(WST.U('home/goods/spellSaleByPage'),params,function(data,textStatus){
        var json = WST.toJson(data);
        if(json.status==1 && json.Rows){
            var gettpl = document.getElementById('tblist').innerHTML;
            laytpl(gettpl).render(json.Rows, function(html){
