...
class Goods extends CGoods{
      ...
      *  上架商品列表
      */
	public function saleByPage($isSpell=0){
		...
		if($isSpell>0){
            $rs = $this->alias('m')
                ...
                ->field('m.goodsId,m.goodsName,m.goodsImg,m.goodsSn,isSale,m.isBest,m.isHot,m.isNew,m.isRecom,m.goodsStock,m.saleNum,m.shopPrice,m.isSpec,s.num,s.price')
                ->order('saleTime', 'desc')
...
//        die();
        ...
        $data['goodsCatIdPath'] = implode('_',$goodsCats)."_";
        $data['isSpec'] = ($specsIds!='')?1:0;
        $dataSpell['num']=$data['num'];
        $dataSpell['price']=$data['price'];
        $data['isSpell']=1;
        Db::startTrans();
        try{
            $shop = model('shops')->get($shopId);
            ...
            if(false !== $result){
                ...
                //商品拼团信息
                $dataSpell['goods_Id']=$goodsId;
                Db::name('spell')->insert($dataSpell);
	 ...
	 * 编辑商品资料
	 */
	public function edit($isSpell=0){
		$shopId = (int)session('WST_USER.shopId');
        ...
        try{
			...
			if($isSpell>0){
                //商品拼团信息
                $dataSpell['goods_Id']=$goodsId;
                Db::name('spell')->where(['goods_Id'=>$goodsId])->update($dataSpell);
            }
			if(false !== $result){
				/**
	...
	public function getById($goodsId){
		...
		if(!empty($rs)){
			...
			//获取商品描述-图片
			if($rs['goodsDesc']!='')$rs['goodsDesc'] = explode(',',$rs['goodsDesc']);
			//获取规格值
