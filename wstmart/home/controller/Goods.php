...
class Goods extends Base{
    ...
    */
     ...
     *  上架拼团商品列表
     */
    public function spellSale(){
        return $this->fetch('shops/goods/list_spell_sale');
    }
    /**
     * 获取上架拼团商品列表
     */
    public function spellSaleByPage(){
        $m = new M();
        $rs = $m->saleByPage(1);
        $rs['status'] = 1;
        return $rs;
     ...
     * 跳去新增拼团页面
     */
    public function spellAdd(){
        $m = new M();
        $object2 = $m->getEModel('goods');
        $object1 = $m->getEModel('spell');
        $object = array_merge($object2, $object1);
        $object['goodsSn'] = WSTGoodsNo();
        $object['productNo'] = WSTGoodsNo();
        $object['goodsImg'] = WSTConf('CONF.goodsLogo');
        $data = ['object'=>$object,'src'=>'add'];
        return $this->fetch('shops/goods/spell_edit',$data);
    }
    /**
     * 新增商品
     */
     ...
     * 跳去拼团编辑页面
     */
    public function spellEdit(){
        $m = new M();
        $object = $m->getById(input('get.id'));
        $spellData=Db::name('spell')->where(['goods_Id'=>input('get.id')])->find();
        $object['num']=$spellData['num'];
        $object['price']=$spellData['price'];
//        echo "<pre>";
//        var_dump($object);die();
        if($object['goodsImg']=='')$object['goodsImg'] = WSTConf('CONF.goodsLogo');
        $data = ['object'=>$object,'src'=>input('src')];
    ...
    public function ajax_uploader(){
        ...
        foreach($files as $file){
            ...
            if($info){
                ...
                $file1 = 'public' . DS . 'uploads/' . $info->getSavename();
                $file12= str_replace('\\', '/', $file1);
                $path['name']=$file12;
            }else{
                // 上传失败获取错误信息
