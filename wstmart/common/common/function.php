 ...
 * APP-生成数据返回值
 */
function SZXReturn($msg,$code = 2,$data = []){
    $rs = ['code'=>$code,'msg'=>$msg,'data' =>$data];
//    if($data!='')$rs['data'] = $data;
...
function WSTIsPhone($phoneNo){
	$reg = "/^1[3,4,5,6,7,8,9][\d]{9}$|^14[5,7]{1}\d{8}$|^15[^4]{1}\d{8}$|^17[0,6,7,8]{1}\d{8}$|^18[\d]{9}$/";
...
function WSTGoodsCats($parentId = 0,$isFloor = -1){
	...
	return $dbo->field("catName,catId,catImg")->order('catSort asc')->select();
}
