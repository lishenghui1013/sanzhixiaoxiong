...
class Orders extends Base{
	...
	public function submit(){
		$addressId = (int)input('post.s_addressId');//地址id
		$deliverType = ((int)input('post.deliverType')!=0)?1:0;//送货方式 自提 运输
		$isInvoice = ((int)input('post.isInvoice')!=0)?1:0;//是否 发票
		$invoiceClient = ($isInvoice==1)?input('post.invoiceClient'):'';
		$payType = ((int)input('post.payType')!=0)?1:0;//支付方式 货到付  在线付
