...
//    		}
    		...
    		if($rs['userType']>=1){
                ...
                //判断是否托管
                $isTrusteeship=Db::name('trusteeship')->where(['shopId'=>$shop['shopId'],'trpositionId'=>37,'dataFlag'=>1])->order('trId desc')->find();
                if(!empty($isTrusteeship)){
                    $time=date('Y-m');
                    $Trusteeship = date('Y-m', strtotime($isTrusteeship['endDate']));
                    $date = date('Y-m', strtotime($isTrusteeship['createTime']));
                    if($time<=$Trusteeship && $date != $time){
                        session('tuoguan',1);
                    }else{
                        session('tuoguan', null);
                    }
                }
    		}
    		//记录登录日志
