...
class AdPositions extends Base{
	...
	public function pageQuery(){
		$arr=$this->table('szx_trusteeship')->where(['shopId'=>'-1','dataFlag'=>1])->column('trpositionId');
		//print_r($arr);
		$str=implode(',',$arr);
		//echo $str;
		$where=[];
		$where['dataFlag']=1;
		$where['positionId']=array('not in',$str);
		return $this->where($where)->field(true)->order('apSort asc,positionId asc')->paginate(input('pagesize/d'));
	}
