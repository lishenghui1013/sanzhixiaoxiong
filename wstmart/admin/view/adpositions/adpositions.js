        ...
        columns: [
          ...
          { display: '位置代码', name: 'positionCode', isSort: false},
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	        	var h = "";
				if(WST.GRANT.GGWZ_02 && rowdata['positionType']==1){
                        h += "<a href='"+WST.U('admin/AdPositions/hosting','id='+rowdata['positionId'])+"'>托管</a> ";
                }
...
function editInit1(){
    ...
    $('#hostForm').validator({
            fields: {
				...
				timeunit: {
                  rule:"required",
                  msg:{required:"请输入年/月/日单位"},
                  ok:"",
                },
				proportion: {
                  rule:"required",
                  msg:{required:"请输入收益比例"},
                  ok:"",
                },
                monthNum: {
                  rule:"required",
                  msg:{required:"请输入截止时间"},
                  ok:"",
                }
            },
          valid: function(form){
            var params = WST.getParams('.ipt');
            var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(WST.U('admin/Adpositions/addhosting'),params,function(data,textStatus){
              layer.close(loading);
              var json = WST.toAdminJson(data);
              if(json.status=='1'){
                  WST.msg("操作成功",{icon:1});
