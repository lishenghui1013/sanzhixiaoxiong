...
function initGrid(){
	...
	grid = $("#maingrid").ligerGrid({
		url:WST.U('admin/agent/listupload'),
		pageSize:WST.pageSize,
		pageSizeOptions:WST.pageSizeOptions,
		height:'99%',
        width:'100%',
        minColToggle:6,
		parms: [{name:"id",value:positionid},{name:"shopId",value:shopId}],   //提交到服务器的参数
        rownumbers:true,
        columns: [
          ...
          { display: '点击数', name: 'adClickNum', isSort: false},
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	        	var h = "";
	            if(WST.GRANT.GGWGL_01)h += "<a href='"+WST.U('admin/agent/toupload','id='+rowdata['adId'])+'&positionid='+rowdata['adPositionId']+'&shopId='+rowdata['shopId']+"'>修改</a> ";
	            if(WST.GRANT.GGWGL_02)h += "<a href='javascript:toDel(" + rowdata['adId'] + ")'>删除</a> "; 
	            return h;
	        }}
        ]
    });
          ...
          valid: function(form){
            ...
            $.post(WST.U('admin/Ads/'+((params.adId==0)?"add":"edit")),params,function(data,textStatus){
              ...
              var json = WST.toAdminJson(data);
                ...
                if(json.status=='1'){
                  WST.msg("操作成功",{icon:1});
                  location.href=WST.U('admin/agent/adsupload','id='+position+'&shopId='+shopId);
              }else{
                    WST.msg(json.msg,{icon:2});
              }
            });
